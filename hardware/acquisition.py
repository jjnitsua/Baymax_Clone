...
INT_PIN  = 17     # GPIO pin connected to ADXL345 INT1 (BCM numbering)
...
def init_adxl345_fifo(bus):
    ...
    bus.write_byte_data(ADDR, 0x2F, 0x02)        # INT_MAP: watermark → INT1
    bus.write_byte_data(ADDR, 0x2E, 0x02)        # INT_ENABLE: watermark interrupt on
...
# ── Interrupt callback (lgpio signature: chip, gpio, level, timestamp) ──
def on_watermark(chip, gpio, level, timestamp):
    if level != 1:   # only act on rising edge
        return
...
# open GPIO chip (chip 0 = /dev/gpiochip0 on Pi 5)
h = lgpio.gpiochip_open(0)
lgpio.gpio_claim_input(h, INT_PIN)

# register rising-edge callback
cb = lgpio.callback(h, INT_PIN, lgpio.RISING_EDGE, on_watermark)

print("Acquisition running — waiting for ADXL345 interrupts (Ctrl+C to stop)")

try:
    while True:
        time.sleep(1)   # main thread sleeps; interrupt drives all reads
finally:
    cb.cancel()
